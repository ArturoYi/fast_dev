...
本工具**不支持** Flutter 分辨率目录（`2.0x`、`3.0x`、`1.5x` 以及任何 `数字 + x` 目录）。

- **不会**识别、**不会**合并到基准路径、**不会**按设备 DPR 选图
- `assets/images/2.0x/logo.png` 只是普通子目录，会生成独立成员，不是 `logo` 的 2 倍图
- **不要**按多倍图目录来放资源，也不要把这种模式当成生成器功能

只用一份基准文件，例如 `assets/images/logo.png`。
:::
...
**不支持** `2.0x` / `3.0x` 分辨率目录。它们不会合并到基准路径，见页首声明。
