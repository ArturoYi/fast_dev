...
它读 `pubspec.yaml` 里的 `flutter.assets` 和 `flutter.fonts`，生成类型安全的 Dart 引用。生成文件进仓库，工具本身不进用户包。

如果希望一些业务相关的能力，可以用 [Fast Package](https://github.com/ArturoYi/fast_package)。

## 怎么划分

打包清单还是 `pubspec.yaml` 的 `flutter.assets` / `flutter.fonts`。Flutter 打进 APK / IPA 的，也是这份。

工具怎么跑（输出目录、类名、命名风格、变体目录）写在项目根的 `fast_dev_config.yaml`。建议自己建这份文件，项目里大家看到的是同一套约定。

两边对不上，要到运行时才会发现漏资源。`pubspec.yaml` 没写的路径或字体族，生成代码里也不会有。
